...
from FFCLIP_model.model_attn_aff_voc_seg import FFCLIP
...
parser.add_argument("--model_path", default="/your/path/FFCLIP/FFCLIP_model_iter_30000.pth", type=str, help="model_path")
...
def main(cfg):
    ...
    FFCLIP_model = FFCLIP(num_classes=cfg.dataset.num_classes,
    ...
    FFCLIP_model.load_state_dict(state_dict=trained_state_dict, strict=False)
    FFCLIP_model.eval()

    gts, preds, msc_preds, cams, preds_hist, msc_preds_hist, cams_hist = validate(model=FFCLIP_model, dataset=val_dataset, test_scales=[1, 0.75])
