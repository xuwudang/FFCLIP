...
parser.add_argument("--config",
                    default='/your/path/FFCLIP/configs/voc_attn_reg.yaml',
                    type=str,
                    help="config")
...
def train(cfg):
    ...
    logging.info('\nNetwork config: \n%s'%(FFCLIP_model))
    param_groups = FFCLIP_model.get_param_groups()
    FFCLIP_model.cuda()
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        segs, cam, attn_pred = FFCLIP_model(inputs.cuda(), img_name)
        ...
        if (n_iter + 1) % cfg.train.eval_iters == 0:
            ckpt_name = os.path.join(cfg.work_dir.ckpt_dir, "FFCLIP_model_iter_%d.pth"%(n_iter+1))
            logging.info('Validating...')
            if (n_iter + 1) > 26000:
                torch.save(FFCLIP_model.state_dict(), ckpt_name)
            seg_score, cam_score = validate(model=FFCLIP_model, data_loader=val_loader, cfg=cfg)
