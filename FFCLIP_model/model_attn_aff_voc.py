...
class FFCLIP(nn.Module):
    def __init__(self, num_classes=None, clip_model=None, embedding_dim=256, in_channels=512, dataset_root_path=None,
                 device='cuda'):
        super().__init__()
    ...
    def forward(self, img, img_names='2007_000032', mode='train'):
        ...
        for i, img_name in enumerate(img_names):
            ...
            with torch.no_grad():
                ...
                if mode == 'train' or self.iter_num > 10000:  # Apply WFF after some training
                    # Extract edge features from the original image
                    edge_feature = extract_edge_feature(img[i].unsqueeze(0))
                    edge_feature = edge_feature.repeat(1, f_c, 1, 1).cuda()
                    
                    # Reshape cam_labels for WFF processing
                    cam_labels_2d = cam_labels.unsqueeze(0).unsqueeze(0)  # [1, 1, H, W]
                    cam_labels_2d = F.interpolate(cam_labels_2d.float(), size=(f_h, f_w), mode='bilinear', align_corners=False)
                    cam_labels_2d = cam_labels_2d.repeat(1, f_c, 1, 1)  # [1, C, H, W]
